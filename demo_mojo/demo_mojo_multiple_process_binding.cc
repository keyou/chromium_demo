...
class ApiImpl : public demo::demo_mojo::mojom::Api {
 public:
  explicit ApiImpl(mojo::PendingReceiver<Api> receiver)
      : receiver_(this, std::move(receiver)), associated_receiver_(nullptr) {}
  explicit ApiImpl(mojo::PendingAssociatedReceiver<Api> receiver)
 ...
 private:
  ...
  mojo::AssociatedReceiver<Api> associated_receiver_;
};
...
class Test3Impl : public demo::demo_mojo::mojom::Test3 {
 public:
  explicit Test3Impl(mojo::PendingReceiver<Test3> receiver)
      : receiver_(this, std::move(receiver)) {}
  void GetApi(mojo::PendingReceiver<Api> api) override {
    LOG(INFO) << "Test3 run: GetApi";
    api_ = std::make_unique<ApiImpl>(std::move(api));
  }
  void SetApi2(mojo::PendingRemote<Api2> api2) override {
...
class Test32Impl : public demo::demo_mojo::mojom::Test32 {
 public:
  explicit Test32Impl(mojo::PendingReceiver<Test32> receiver)
      : receiver_(this, std::move(receiver)) {}
  void GetApi(mojo::PendingAssociatedReceiver<Api> api) override {
    LOG(INFO) << "Test32 run: GetApi";
    api_ = std::make_unique<ApiImpl>(std::move(api));
  }
  void SetApi2(mojo::PendingAssociatedRemote<Api2> api2) override {
...
class Interface1Impl : public Interface1 {
 public:
  explicit Interface1Impl(mojo::PendingReceiver<Interface1> receiver)
      : receiver_(this, std::move(receiver)) {}
  void Hello(const std::string& who) override {
 ...
 private:
  mojo::Receiver<Interface1> receiver_;
};

class Interface2Impl : public Interface2 {
 public:
  explicit Interface2Impl(mojo::PendingReceiver<Interface2> receiver)
      : receiver_(this, std::move(receiver)) {}
 ...
 private:
  mojo::Receiver<Interface2> receiver_;
};
...
class InterfaceBrokerImpl : public InterfaceBroker {
 public:
  explicit InterfaceBrokerImpl(mojo::PendingReceiver<InterfaceBroker> receiver)
      : receiver_(this, std::move(receiver)) {}
  ...
  void AddMap(const std::string& name) {
    binders_.emplace(
        name, base::BindRepeating([](mojo::ScopedMessagePipeHandle handle) {
          new InterfaceImplT(mojo::PendingReceiver<InterfaceT>(std::move(handle)));
        }));
  }
 ...
 private:
  ...
  BinderMap binders_;
  mojo::Receiver<InterfaceBroker> receiver_;
};
...
void MojoProducer() {
  ...
  // C++ binding API test
  {
    ...
    auto* test_ptr = new mojo::Remote<Test>(mojo::PendingRemote<Test>(std::move(pipe), 0));
    auto& test = *test_ptr;
    test->Hello("World!");
  ...
  // 使用mojo接口发送其他mojo接口
  {
    ...
    mojo::Remote<Api> api(mojo::PendingRemote<Api>(std::move(api_pipe.handle0), 0));
    test3->GetApi(mojo::PendingReceiver<Api>(std::move(api_pipe.handle1)));
    ...
    new Api2Impl(mojo::PendingReceiver<Api2>(std::move(api2_pipe.handle1)));
    test3->SetApi2(mojo::PendingRemote<Api2>(std::move(api2_pipe.handle0), 0));
  ...
  // Interfaces),关联接口只需要使用一个pipe即可,用来避免多个pipe导致的多接口间调用顺序无法保证的问题
  {
    ...
    mojo::ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(&handle0,
                                                                      &handle1);
    mojo::AssociatedRemote<Api> api(
        mojo::PendingAssociatedRemote<Api>(std::move(handle0), 0));
    test32->GetApi(mojo::PendingAssociatedReceiver<Api>(std::move(handle1)));
    ...
    mojo::ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(
        &handle00, &handle11);
    new Api2Impl(mojo::PendingAssociatedReceiver<Api2>(std::move(handle11)));
    test32->SetApi2(mojo::PendingAssociatedRemote<Api2>(std::move(handle00), 0));
  ...
  // 封装出通用的 InterfaceBroker 以便支撑任何新的接口
  {
    ...
    mojo::Remote<Interface1> interface1(
        mojo::PendingRemote<Interface1>(std::move(interface1_pipe.handle0), 0));
    ...
    mojo::Remote<Interface2> interface2(
        mojo::PendingRemote<Interface2>(std::move(interface2_pipe.handle0), 0));
