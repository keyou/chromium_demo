...
int main(int argc, char** argv) {
  ...
  // 也可以使用下面的方法。它们的区别仅在于MessageLoop对外暴露了更多的内部接口。
...
#else  // DEMO_TASK
int main(int argc, char** argv) {
  base::Thread thread("timer");
  thread.Start();
  base::RepeatingTimer timer_;
  timer_.SetTaskRunner(thread.task_runner());
  ...
  scoped_refptr<base::TaskRunner> task_runner_ = base::CreateTaskRunner({
      base::ThreadPool(),
